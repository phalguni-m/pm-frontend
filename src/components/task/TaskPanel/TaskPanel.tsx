          ...
          {draftStatus === "waiting" && (
            <div className={styles.fieldFullWidth}>
              <Field label="Delay cause" htmlFor="task-panel-cause" required errorMessage={justEnteredWaiting ? "Choose a cause before saving." : undefined}>
                <DropdownPill
                  label={delayCauses.find((c) => c.id === draftDelayCauseId)?.name ?? "Choose a cause"}
                  items={delayCauses.map((cause) => ({ id: cause.id, label: cause.name, selected: cause.id === draftDelayCauseId }))}
                  onSelect={(id) => {
